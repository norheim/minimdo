...
from modeling.arghandling import Encoder, decode, encode  
...
class ResidualProjectable():
    ...
    def functional(self, x_initial=None, **kwargs):
        RS = residual_solver(self.R, self.solvevar_encoder, 
                            x_initial=x_initial, **kwargs)
        return RS
...
def merge_with_coupling(*projectables):
    ...
    for F in all_functions:
        ...
        res = EncodedFunction(function, Encoder(mergedin_vars, 
                                                mergedin_shapes))
        residuals += (res,)
    R = concatenate_residuals(residuals)
    return ResidualProjectable(R, Encoder(coupled_vars, 
                                          coupled_shapes))
